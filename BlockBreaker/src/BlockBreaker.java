...
class Title extends block_breaker//시작 화면
{
   ...
   String st1=new String("Java Programing");
   String st2=new String("Homework #4");
...
class Play extends block_breaker//실행 화면
{
   LinkedList<Rect> rt=new LinkedList<Rect>();//블록 클래스의 연결리스트
   LinkedList<Ball> ball=new LinkedList<Ball>();//공 연결리스트
   AudioClip audio2;
   AudioClip audio3;
   AudioClip audio4;
   
   int x=350, y=670;
   int xsize=130, ysize=25;//라켓의 정보
   ...
   Play()
   {
      ...
      URL url2 = getClass().getResource("./sounds/attack.wav");
      audio2 = Applet.newAudioClip(url2);
      
      URL url3 = getClass().getResource("./sounds/break.wav");
      audio3 = Applet.newAudioClip(url3);
      
      URL url4 = getClass().getResource("./sounds/level_up.wav");
      audio4 = Applet.newAudioClip(url4);
   }
   
   void divideRacket()//라켓 5분할
   {
      n=xsize/5;
      x1=x+n;
      x2=x1+n;
   ...
   void resetball()//공의 속도, 방향 바꿈
   {
      ...
      if (rt.size()<=0)//블록이 다 부숴지면 다음 스테이지
      {
         stage++;
         audio4.play();
         resetstage();
   ...
   void collision_block()
   {
      ...
      {
         for (int i=0; i<4; i++)//블록과의 충돌 검사하고 해당 충돌 벽돌 지우기
         {
   ...
   boolean collision(Point p, int _x, int _y, Point bp)//충돌 검사
   {   
   ...
   void draw (Graphics g)//공과 벽돌 그리기
   {
      ...
      for (Rect r: rt)
      {
         if (r.yellow)
         {
            GradientPaint gp=new GradientPaint(r.pt.x,r.pt.y,Color.YELLOW,r.pt.x+bx_size,r.pt.y+by_size,Color.ORANGE);
            g2.setPaint(gp);
         }
         else
         {
            GradientPaint gp=new GradientPaint(r.pt.x,r.pt.y,Color.BLUE,r.pt.x+bx_size,r.pt.y+by_size,Color.CYAN);
...
class Over extends block_breaker//종료 화면
{
   int your_score=0;
   int score=0;
   Play play;
   ...
   String st2=new String("HIGH SCORE: "+your_score);
...
class Game extends JPanel implements KeyListener, Runnable
{
   ...
   Thread t_title=new Thread(this);
   int flag=1;
   int mode=GAME_START;
   ...
   Game()
   {
      ...
      URL url3 = getClass().getResource("./sounds/over.wav");
   ...
   protected void paintComponent(Graphics g)
   {
      ...
      if (mode==GAME_START)
      {
         ...
         g.setFont(t.font3);
         g.setColor(Color.black);
         g.drawString(t.st4, 235, 600);
         g.setColor(Color.red);
         g.drawString(t.st4, 232, 600);
      }
      ...
      else if (mode==GAME_OVER)
      {
         ...
         if (o.your_score>high_score)
            high_score=o.your_score;
         
         o.st2="HIGH SCORE: "+high_score;
         o.draw(g);
         g.setFont(o.font3);
         g.setColor(Color.black);
         
         g.drawString(o.st4, 240, 600);
         g.setColor(Color.red);
         g.drawString(o.st4, 237, 600);
      }
   }
   ...
   public void keyPressed(KeyEvent e) {
      ...
      if (e.getKeyCode()==KeyEvent.VK_SPACE)
      {
         ...
         else if (mode==GAME_OVER)//게임이 끝났을 때 초기화, 스테이지 1로 설정
         {
            ...
            for (int y=0; y<3; y++)
            {
               for (int x=0; x<3; x++)
               {
                  Point c=new Point(25+(p.bx_size*x)+(5*x),25+(5*y)+(p.by_size*y));
                  boolean temp=false;
                  if (c1%5==0)
                     temp=true;
                  else
   ...
   public void run() {
      ...
      while(true)
      {
         ...
         {
            a=100;
            if (flag==1) {t.st4=""; flag=0;}
            else if (flag==0)
            {
               t.st4="PRESS SPACEBAR TO PLAY!";
               flag=1;
            }
         }
         else if (mode==GAME_PLAY)
         ...
         {
            a=100;
            if (flag==1) {o.st4=""; flag=0;}
            else if (flag==0)
            {
               o.st4="PRESS SPACEBAR!";
               flag=1;
            }
            a=100;
         }
